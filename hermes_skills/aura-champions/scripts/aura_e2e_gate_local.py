#!/usr/bin/env python3
# aura_e2e_gate_local.py
# Local variant of aura_e2e_gate.py that uses local psql instead of docker exec.
# Same test cases, same exit codes.
import sys, os, json, time, hmac, hashlib, base64, urllib.request, urllib.error, subprocess, argparse, uuid

# ---- config (mirrors compose env) ----
HMAC_SECRET = os.environ.get("HMAC_MASTER_SECRET", "4fgEnKlbAYEUhztz4bL3F4w3Iupaa5MAlbg7YgV3ktokXGTnFLbGSvqFu7ZebeFf").encode()
...
PGPASSWORD_ENV = {"PGPASSWORD": "HMKR6l0pAWWBgImPr_KErfw5ai0XAMtd8QycXo8SShU"}

BID = "11111111-1111-1111-1111-111111111111"
UID = "22222222-2222-2222-2222-222222222222"
REAL_CARD = "AURA00000032e6d5c492"   # card_id 50 from seed catalog
...
def sign(body, ts):
    return hmac.new(HMAC_SECRET, (b64(body) + "|" + str(ts)).encode(), hashlib.sha256).hexdigest()
...
def db(sql, timeout=15):
    """Run a SQL query against local PostgreSQL."""
    env = dict(os.environ, **PGPASSWORD_ENV)
    r = subprocess.run(
        [PSQL, "-h", "localhost", "-p", "5432", "-U", "aura", "-d", "aura", "-t", "-c", sql],
        capture_output=True, text=True, timeout=timeout, env=env
    )
    return r.returncode, r.stdout.strip()

def udp():
    return urllib.request.urlopen(NFC + "/health", timeout=5).status
...
def flow_tournament():
    ...
    if not mid:
        # Try to find it in DB as fallback
        rc2, rows = db(f"SELECT id FROM matches WHERE tournament_id='{TID}' ORDER BY created_at DESC LIMIT 1;")
        if rows:
            mid = rows.strip().splitlines()[0].strip()
    ok("createMatch has match id", bool(mid), f"-> {mid}")
...
def flow_nfc():
    ...
    test_card = "GATE" + uuid.uuid4().hex[:12].upper()  # unique per run
    try:
        hcode = udp()
    except Exception:
        hcode = -1
...
def main():
    print("=== AURA CHAMPIONS E2E GATE (Local psql) ===")
    flow_battle()
    flow_tournament()
    flow_nfc()
    flow_contract()
