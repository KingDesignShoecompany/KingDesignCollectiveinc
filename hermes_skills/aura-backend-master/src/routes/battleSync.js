...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const controllingTeamIndex = Array.isArray(battle.players)
      ...
      : 0;
    const battleState = battle.state && typeof battle.state === 'object' ? battle.state : { teams: [] };
    const trinityResult = checkTrinity(battleState, controllingTeamIndex >= 0 ? controllingTeamIndex : 0);
